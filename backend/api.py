...
async def submit_agent(submission: SubmissionCode, current_user: dict = Depends(get_current_user), session: Session = Depends(get_db)):
    ...
    league_folder = team.league.folder.lstrip('/') if team.league.folder else f"leagues/user/{team.league.name}"

    if not database.allow_submission(session, team.id):
        logger.error(f'Team "{team_name}" has exceeded the maximum number of submissions per minute.')
    ...
    try:
        ...
        logger.error(f"Error running validation simulation: {str(e)}")
        return ErrorResponseModel(status="error", message=f"An error occurred while validating the submission: {str(e)}")
